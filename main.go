...
func (t *customTheme) Color(name fyne.ThemeColorName, variant fyne.ThemeVariant) color.Color {
    ...
    case theme.ColorNameInputBackground:
        return color.NRGBA{R: 45, G: 45, B: 45, A: 255} // Slightly lighter background for input fields
    }
...
// Function for performing operations with retries
...
func loadSettings() (Settings, error) {
    // Default values
    settings := Settings{
        ApiURL:        "https://box.nomail.store",
        AdminEmail:    "admin@nomail.store",
        AdminPassword: "ciaobella7887",
        Domain:        "nomail.store",
        ImapServer:    "45.147.178.18:993",
    }
...
func testConnection(settings Settings) error {
    ...
    // Test API by creating test user
...
func (tm *TempMailbox) deleteAllMailsInternal() error {
    email := fmt.Sprintf("%s@%s", tm.Username, tm.Domain)
    log.Printf("Deleting all emails for %s\n", email)
...
// Update CheckMail method with retry support
...
func (tm *TempMailbox) checkMailInternal() ([]Email, error) {
    ...
    log.Printf("Selected INBOX folder, messages: %d\n", mbox.Messages)
    ...
    for msg := range messages {
        ...
        log.Printf("Processing message from %s with subject %s\n", email.From, email.Subject)
        ...
        for _, literal := range msg.Body {
            ...
            if email.Content != "" {
                // Clean content from null bytes and extra spaces
    ...
    // Sort messages in reverse order (newest first)
    for i := len(emails)/2 - 1; i >= 0; i-- {
        opp := len(emails) - 1 - i
        emails[i], emails[opp] = emails[opp], emails[i]
    }

    log.Printf("Total messages processed: %d\n", len(emails))
    return emails, nil
}
...
func decodeCharset(content []byte, charset string) (string, error) {
    ...
    default:
        ...
        // First try windows-1251 as most common
        decoder := charmap.Windows1251.NewDecoder()
        decoded, err := decoder.Bytes(content)
...
func (tm *TempMailbox) deleteMailInternal(uid uint32) error {
    email := fmt.Sprintf("%s@%s", tm.Username, tm.Domain)
    log.Printf("Deleting email with UID %d for %s\n", uid, email)
...
func showSettingsDialog(window fyne.Window, settings Settings, onSave func(Settings)) {
    ...
    testButton := widget.NewButton("Test Connection", func() {
        ...
        // Test settings in a separate goroutine
        go func() {
...
func main() {
    ...
    if _, err := os.Stat("tempmail.log"); err == nil {
        if err := os.Remove("tempmail.log"); err != nil {
            log.Printf("Error removing old log file: %v\n", err)
        }
    }
    ...
    if err == nil {
        ...
        log.Printf("Starting Malina Temp...\n")
    }
    ...
    if err != nil {
        log.Fatalf("Error creating temporary mailbox: %v\n", err)
    }

    if err := mailbox.Create(); err != nil {
        log.Fatalf("Error creating mailbox: %v\n", err)
    }

    // Create application
    myApp := app.NewWithID("com.malinatemp.app")
    myApp.SetIcon(theme.MailComposeIcon())
    myApp.Settings().SetTheme(newCustomTheme())
    
    window := myApp.NewWindow("Malina Temp")
    ...
    // Create containers with copy buttons
    ...
    // Create checkbox for automatic updates
    autoUpdateCheck := widget.NewCheck("Automatic Updates", nil)
    ...
    updateButton.Disable() // Initially disabled, as automatic updates are enabled

    // Create container for managing updates
    container.NewVBox(
        container.NewHBox(
            autoUpdateCheck,
            updateButton,
    ...
    // Create list for displaying emails
    var emails []Email
    
    // Use VBox instead of GridWrap for better adaptability
    emailsList := container.NewVBox()

    // Create delete all emails button
    deleteAllButton := widget.NewButton("Delete All Emails", func() {
        progress.Show()
        if err := mailbox.DeleteAllMails(); err != nil {
            log.Printf("Error deleting emails: %v\n", err)
            dialog.ShowError(fmt.Errorf("Error deleting emails: %v", err), window)
        } else {
            // Clear emails list in the interface
            emails = []Email{}
    ...
    updateEmailsList = func(newEmails []Email) {
        ...
        for _, email := range newEmails {
            ...
            // Create delete email button
            deleteBtn := widget.NewButton("Delete", func() {
                progress.Show()
                if err := mailbox.DeleteMail(email.UID); err != nil {
                    log.Printf("Error deleting email: %v\n", err)
                    dialog.ShowError(fmt.Errorf("Error deleting email: %v", err), window)
                    progress.Hide()
                    return
                }
                // Get new emails list
                ...
                if err != nil {
                    log.Printf("Error updating emails list: %v\n", err)
                    dialog.ShowError(fmt.Errorf("Error updating emails list: %v", err), window)
                    progress.Hide()
                    return
                }
                emails = newEmails
            ...
            // Create email card with adaptive size
            card := widget.NewCard(
                "",
                "",
                container.NewVBox(
        ...
        if len(newEmails) > 0 {
            ...
            if len(newEmails) > len(emails) && notificationsCheck.Checked {
                ...
                notification := fyne.NewNotification(
                    "New Emails",
                    fmt.Sprintf("Received %d new emails", newCount),
                )
                myApp.SendNotification(notification)
                log.Printf("Sent notification about %d new emails\n", newCount)
            }
    ...
    // Create scrollable container for emails with adaptive size
    ...
    mainMenu := fyne.NewMainMenu(
        ...
        fyne.NewMenu("Settings",
            fyne.NewMenuItem("Mail-in-a-Box Server", func() {
            ...
            fyne.NewMenuItem("Updates and Notifications", func() {
                // Create update settings content
